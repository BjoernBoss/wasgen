...
namespace wasm {
	...
	*	must implement various interaction member-functions for it */
	...
	concept IsModule = requires(Type state, const std::u8string_view id, const wasm::Limit limit, const wasm::Exchange exchange, const wasm::Param param, wasm::Type type, uint32_t c) {
		...
		{ typename Type::Module{ state }.getSink(std::declval<const typename Type::Function&>()) } -> std::same_as<typename Type::Sink>;
	};
	...
	struct Module {
		...
		wasm::Global<Writer> global(const std::u8string_view& id, wasm::Type type, const wasm::Exchange& exchange = {}) {
		...
		wasm::Function<Writer> function(const std::u8string_view& id, const wasm::Prototype<Writer>& prototype, const wasm::Exchange& exchange = {}) {
		...
		wasm::Sink<Writer> sink(const wasm::Function<Writer>& fn) {
			return wasm::Sink<Writer>{ std::move(self.getSink(fn.self)) };
		}
	};
}
